...
# --- 1. 初始化設定 ---
st.set_page_config(page_title="富邦 D&O 補漏採集器 (V11.0)", layout="wide", page_icon="🛡️")
st.title("🛡️ D&O 智能核保 - 缺漏資料補足系統 (智能修補版)")
...
def get_all_db_data():
    """取得資料庫所有資料 (含 financial_data 內容，用於檢查完整性)"""
    try:
        all_data = []
        start = 0
        batch_size = 1000 
...
def fetch_finmind_data_history(stock_code):
    try:
        ...
        if data_balance:
            assets, liabs, cur_assets, cur_liabs = {}, {}, {}, {}
            ...
            for d in sorted_dates:
                ...
                if d in liabs:
                    ...
                    if assets[d] > 0:
                        parsed_data["負債比"][q_key] = f"{(l_val / assets[d]) * 100:.2f}%"
        ...
        if data_cash:
            targets = ['CashFlowFromOperatingActivities', 'CashFlowsFromOperatingActivities', 'NetCashFlowsFromUsedInOperatingActivities', 'NetCashInflowFromOperatingActivities']
    ...
    try:
        ...
        if force_finmind:
             ...
             if fm_data_list:
                 ...
                 return False, f"❌ FinMind 亦無資料: {stock_name_tw}"
        else:
            # 一般流程
...
# --- 5. UI 介面 ---
tab1, tab2, tab3 = st.tabs(["🔍 補漏監控", "🚑 資料修補 (Fix)", "📝 單筆手動"])
...
with tab2:
    ...
    st.info("此功能會掃描資料庫中「欄位過少」或「只有一季資料」的興櫃公司，並強制使用 FinMind 歷史模式重抓。")
    
    if st.button("🔍 1. 掃描需修補名單"):
        with st.spinner("分析資料庫品質中..."):
            all_data = get_all_db_data() # 取得所有資料
            repair_list = []
            ...
            for item in all_data:
                ...
                # 判斷標準：
                # 1. 如果資料來源是 "FinMind (興櫃備援)"
                # 2. 且 EPS 只有 1 個欄位 (除了 "項目" 之外只有 1 個 Key)
                is_finmind = False
                data_count = 0
                
                if isinstance(fdata, list):
                    for row in fdata:
                        if "說明" in row and "FinMind" in row.get("說明", ""):
                            is_finmind = True
                        if row.get("項目") == "每股盈餘(EPS)":
                            # 計算 key 數量 (扣除 "項目")
                            data_count = len([k for k in row.keys() if k != "項目"])
                
                # 如果是 FinMind 抓的，且只有 1 季資料 -> 列入修補
                if is_finmind and data_count <= 1:
                    repair_list.append({"code": code, "name": name, "status": "資料不全 (僅1季)"})
            
            if repair_list:
                repair_df = pd.DataFrame(repair_list)
                st.session_state.repair_df = repair_df
                st.warning(f"⚠️ 發現 {len(repair_df)} 家興櫃公司資料不全 (只有最新一季)。")
                st.dataframe(repair_df)
            else:
                st.success("🎉 資料庫品質良好，無須修補。")
    ...
    if 'repair_df' in st.session_state:
        ...
        if st.button(f"🛠️ 2. 強制修補 {len(r_df)} 家"):
            ...
            for i, row in enumerate(r_df.itertuples()):
                ...
                status.text(f"修補中 ({i+1}/{total}): {code} {name} ...")
            ...
            st.success(f"🎉 修補完成！共更新 {fixed_cnt} 家公司資料。")

with tab3:
    st.markdown("### 📝 手動單筆查詢")
    s_in = st.text_input("輸入代號", value="1269")
